"""
Phase 24: Depth-focused training — lambda_depth 대폭 증가 + lambda_diff 감소

Phase 23 문제:
  - ratio=73M: l_depth가 negative이므로 max(l_depth_w, 1e-9)=1e-9 → ratio 폭발
  - lambda_depth=0.3이 l_diff=0.07에 비해 너무 작아 depth 신호가 묻힘
  - adaptive lambda가 negative l_depth에서 동작 안 함

Phase 24 수정:
  1. lambda_depth = 5.0 (17x 증가)
  2. lambda_diff  = 0.05 (확산 loss를 depth loss보다 작게)
     → 학습이 depth ordering에 집중되도록 강제
  3. ratio 계산 fix: abs(l_depth_w) 사용
  4. adaptive lambda 조건도 abs 기준으로 재작성

목표:
  - depth_rank_accuracy > 0.80
  - depth chart에서 VCA 선과 GT 선이 분리되는 패턴 관찰
"""
import argparse
import copy
import json
...
# ─── 기본값 ───────────────────────────────────────────────────────────────────
DEFAULT_LAMBDA_DEPTH  = 5.0    # Phase 24: 17x 증가 (depth 집중)
DEFAULT_LAMBDA_DIFF   = 0.05   # Phase 24: diffusion loss 대폭 감소
...
def debug_multiangle_depth(pipe, vca_layer, dataset, train_procs,
    ...
    for idx in range(len(dataset)):
        sample = dataset[idx]
        frames_np, latents_or_none, depth_orders, meta, *_ = sample
...
def training_step_p21(pipe, vca_layer, latents, encoder_hidden_states,
    ...
    """
    Phase 24: lambda_diff << lambda_depth → depth ordering이 학습 주도.
    entity_masks: (T, 2, S) bool numpy — 16×16 flatten entity spatial masks.
    """
    noise = torch.randn_like(latents)
    ...
    loss = lambda_diff * ld + lambda_depth * l_depth + lambda_ortho * l_ort
    return {
...
def train(args):
    ...
    print(f"[init] device={device}  lambda_depth={args.lambda_depth}  lambda_diff={args.lambda_diff}  alpha={VCA_ALPHA}",
          flush=True)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in loader:
            ...
            latents = encode_frames_to_latents(pipe, frames_np, device)
            # Phase 23: color-qualified prompt per sample
            _, _, full_prompt, _, _ = make_color_prompts(meta)
            tokens = pipe.tokenizer(
                full_prompt, return_tensors="pt", padding="max_length",
                max_length=pipe.tokenizer.model_max_length, truncation=True,
            ...
            step_out = training_step_p21(
                ...
                entity_masks=entity_masks, lambda_diff=args.lambda_diff,
            )
        ...
        # Fix: l_zorder_direct returns negative values (minimization of -sigma)
        # → use abs for ratio so the display is meaningful
        ratio     = l_diff_v / max(abs(l_depth_w), 1e-9)

        # Phase 24: depth-focused → adaptive lambda disabled (depth intentionally dominates)
...
def parse_args():
    ...
    p.add_argument("--save-dir",     default="checkpoints/phase24",    dest="save_dir")
    p.add_argument("--debug-dir",    default="debug/train_phase24",    dest="debug_dir")
