"""
Phase 31: Fix backbone dominance — VCA has no visible effect on generation

Phase 30 root cause:
  - VCA delta is normalized to gamma * |text_out|. With gamma≈0.364 and text_out
    in fp16 (magnitude ~0.05), absolute VCA signal is ~0.018 — too small to
    affect the final 256×256 image.
  - lambda_diff=0.05 means generation quality barely matters, so downstream
    UNet layers never needed to care about VCA.

Phase 31 targeted changes:
  1. VCA_ALPHA = 1.0 (was 0.3) → VCA starts at 100% of text_out magnitude.
  2. DEFAULT_LAMBDA_DIFF = 0.5 (was 0.05) → 10x heavier weight on generation.
  3. Remove LoRA-based text attention alignment (LoRALayer, LoRAAttnProcessor,
     l_attn_mask_loss) entirely. Phase 31 focuses only on forcing VCA to
     actually influence pixel output via higher gamma_init + higher l_diff.
  4. Inference uses the TRAINED gamma (not the fixed VCA_ALPHA) so the
     measured generation difference reflects what training actually learned.
  5. New diagnostic: measure_generation_diff() — per-epoch mean absolute pixel
     diff between Baseline (no VCA) and VCA generation. This is the only
     metric that answers "did VCA affect the image at all?"

목표:
  - generation_diff > 0.01 (normalized) → VCA visible in pixel space
  - depth_rank_accuracy > 0.75 유지
"""
import argparse
import copy
import json
...
from scripts.prompt_identity import (
    make_color_prompts as shared_make_color_prompts,
    make_identity_prompts as shared_make_identity_prompts,
)

# ─── 기본값 ───────────────────────────────────────────────────────────────────
DEFAULT_LAMBDA_DEPTH  = 5.0    # Phase 31: depth 집중 유지
DEFAULT_LAMBDA_DIFF   = 0.5    # Phase 31: 10x ↑ (was 0.05) — 생성 품질 강제
DEFAULT_LAMBDA_ORTHO  = 0.005
DEFAULT_LR            = 5e-5
DEFAULT_EPOCHS        = 60
DEFAULT_T_MAX         = 200
DEPTH_PE_INIT_SCALE   = 0.3
VCA_ALPHA             = 1.0   # Phase 31: 100% of text_out magnitude (was 0.3)
...
def make_identity_prompts(meta: dict) -> tuple:
    return shared_make_identity_prompts(meta)
    ...
    for sigma in sigma_acc:                         # (BF, S, N, Z), requires_grad
        ...
        for fi in range(n):
            ...
            if entity_masks is not None and fi < entity_masks.shape[0]:
                ...
                s_fn = sigma[fi][mf, front, 0].mean()
                s_bf = sigma[fi][mb, back,  Z-1].mean()
                s_ff = sigma[fi][mf, front, Z-1].mean()
                s_bn = sigma[fi][mb, back,  0].mean()
            else:
...
# ─── 핵심: Additive VCA Processor ──────────────────────────────────────

class AdditiveVCAProcessor:
    """
    Phase 21/31: text cross-attention 유지 + VCA를 depth bias로 추가.
    ...
    return text_out + gamma * (|text|/|delta|) * vca_delta

    gamma: learnable scalar initialized to VCA_ALPHA (1.0 in Phase 31).
    Phase 31: gamma_init=1.0 → VCA starts at 100% text_out magnitude.
    """
    def __init__(self, vca_layer: VCALayer, entity_ctx: torch.Tensor,
        ...
        self.alpha = alpha           # backward compat
        # Learnable gamma: initialized to alpha (1.0 in Phase 31)
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None,
        ...
        # Normalize: |vca_delta| = gamma * |text_out|
        with torch.no_grad():
...
class AdditiveVCAInferProcessor:
    ...
    Phase 31: gamma_trained (학습으로 얻은 실제 gamma 값)를 optional로 받음.
    None이면 self.alpha 사용 (기존 동작).
    """
    def __init__(self, vca_layer: VCALayer, entity_ctx: torch.Tensor,
                 orig_processor: AttnProcessor2_0, alpha: float = VCA_ALPHA,
                 gamma_trained: float = None):
        self.vca   = vca_layer
        self.ctx   = entity_ctx
        self.orig  = orig_processor
        self.alpha = alpha
        self.gamma_trained = gamma_trained
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None,
        ...
        text_mag  = text_out.float().abs().mean() + 1e-8
        delta_mag = delta_raw.abs().mean() + 1e-8
        effective_alpha = self.gamma_trained if self.gamma_trained is not None else self.alpha
        vca_delta = delta_raw * (text_mag / delta_mag) * effective_alpha
...
def inject_vca_p21(pipe, entity_ctx: torch.Tensor):
    ...
    # rebuild leaf tensor on device (required by optimizer)
    ...
    print(f"[Phase 31][inject] additive VCA (alpha={VCA_ALPHA}) → {INJECT_KEY}", flush=True)
    return vca_layer, orig_procs


def inject_vca_p21_infer(pipe, vca_layer: VCALayer, entity_ctx: torch.Tensor,
                         gamma_trained: float = None):
    """추론용 Additive VCA 주입. gamma_trained를 주면 학습된 gamma를 사용."""
    unet = pipe.unet
    orig_procs = copy.copy(dict(unet.attn_processors))
    orig_proc  = orig_procs[INJECT_KEY]
    new_proc   = AdditiveVCAInferProcessor(
        vca_layer, entity_ctx, orig_proc, alpha=VCA_ALPHA,
...
# ─── Phase 31: Generation diff measurement ──────────────────────────────

@torch.no_grad()
def measure_generation_diff(pipe, vca_layer, orig_procs, train_procs,
                            probe_meta, probe_entity_ctx, device,
                            height=256, width=256, seed=42):
    """
    VCA가 생성에 실제 차이를 만드는지 정량 측정.
    Returns mean absolute pixel difference between Baseline and VCA generation.
    """
    from PIL import Image as PILImg
    prompt = make_color_prompts(probe_meta)[2]

    # Get trained gamma for inference
    vca_proc = pipe.unet.attn_processors.get(INJECT_KEY)
    trained_gamma = None
    if isinstance(vca_proc, AdditiveVCAProcessor):
        trained_gamma = float(vca_proc.gamma.item())
    ...
    out_base = pipe(prompt=prompt, num_frames=8, num_inference_steps=20,
                   guidance_scale=7.5, height=height, width=width,
                   generator=g, output_type='pil')
    base_frames = [np.array(f).astype(float) for f in out_base.frames[0]]

    # VCA
    orig_proc_ref = orig_procs.get(INJECT_KEY) or AttnProcessor2_0()
    infer_proc = AdditiveVCAInferProcessor(vca_layer, probe_entity_ctx,
                                           orig_proc_ref, alpha=VCA_ALPHA,
                                           gamma_trained=trained_gamma)
    infer_procs = dict(orig_procs)
    infer_procs[INJECT_KEY] = infer_proc
    pipe.unet.set_attn_processor(infer_procs)
    g = torch.Generator(device=device).manual_seed(seed)
    out_vca = pipe(prompt=prompt, num_frames=8, num_inference_steps=20,
                  guidance_scale=7.5, height=height, width=width,
                  generator=g, output_type='pil')
    vca_frames = [np.array(f).astype(float) for f in out_vca.frames[0]]
    ...
    diffs = []
    for bf, vf in zip(base_frames, vca_frames):
        diffs.append(np.abs(bf - vf).mean())
    mean_diff = float(np.mean(diffs)) if diffs else 0.0

    norm_diff = mean_diff / 255.0
    label = "VCA_INVISIBLE" if norm_diff < 0.01 else "VCA_SUBTLE" if norm_diff < 0.05 else "VCA_VISIBLE"
    print(f"  [gen_diff] baseline_vs_vca mean_diff={mean_diff:.2f}px "
          f"({norm_diff:.4f}) [{label}]", flush=True)
    return mean_diff
...
def debug_generation(pipe, vca_layer, orig_procs, train_procs,
    ...
    """
    [GT | Baseline | VCA | VCA swapped] 3-panel GIF + sigma overlay.

    Phase 31: VCA inference uses trained gamma value.
    """
    ...
    # Read trained gamma from the live train processor
    vca_proc = pipe.unet.attn_processors.get(INJECT_KEY)
    trained_gamma = None
    if isinstance(vca_proc, AdditiveVCAProcessor):
        trained_gamma = float(vca_proc.gamma.item())
    ...
    infer_proc = AdditiveVCAInferProcessor(
        ...
        orig_proc_ref, alpha=VCA_ALPHA, gamma_trained=trained_gamma,
    )
    infer_procs = dict(orig_procs)
    infer_procs[INJECT_KEY] = infer_proc
    pipe.unet.set_attn_processor(infer_procs)
    ...
    # 3. VCA with SWAPPED entity context
    ctx_swapped = torch.flip(probe_entity_ctx.clone(), dims=[1])
    ...
    print(f"  [Phase 31][debug] recon → {recon_path.name}  "
...
def debug_multiview(pipe, vca_layer, orig_procs, train_procs,
    ...
    """같은 entity pair를 다양한 카메라 뷰로 시각화."""
    import imageio.v2 as iio2
    from PIL import Image, ImageDraw, ImageFont
    ...
    infer_proc = AdditiveVCAInferProcessor(
        ...
        orig_proc_ref_mv, alpha=VCA_ALPHA, gamma_trained=trained_gamma,
    )
    infer_procs = dict(orig_procs)
    infer_procs[INJECT_KEY] = infer_proc
    pipe.unet.set_attn_processor(infer_procs)
    ...
    if rows:
        ...
        print(f"  [Phase 31][debug] multiview ({len(rows)} views) → {out_path.name}", flush=True)


# ─── 심층 VCA 진단 시각화 ────────────────────────────────────────────────────

def _heat_map(m, lo=None, hi=None):
    """(H, W) float → (H, W, 3) uint8 컬러 히트맵."""
...
def _decode_latents_safe(pipe, latents):
    try:
        ...
        for i in range(lat.shape[0]):
            dec = pipe.vae.decode(lat[i:i+1]).sample
...
def debug_vca_internals(pipe, vca_layer, orig_procs, train_procs,
                        probe_frames_np, probe_meta, probe_entity_ctx,
                        debug_dir: Path, epoch: int, height=256, width=256):
    """Phase 31 심층 진단. denoise trajectory, delta magnitude, ray internals."""
    from PIL import Image, ImageDraw, ImageFont
    import imageio.v2 as iio2
    ...
    if step_keys:
        ...
        if traj_gif:
            ...
            print(f"  [Phase 31][debug] denoise_traj → denoise_traj_epoch{epoch:03d}.gif", flush=True)

    delta_gif = []
    mean_ratio = 0.0
    if (instr_proc.last_vca_delta is not None
            and instr_proc.last_text_out is not None):
        delta  = instr_proc.last_vca_delta.float()
        ttext  = instr_proc.last_text_out.float()
        BF, S, D = delta.shape
        hw = max(1, int(S ** 0.5))

        ratio    = delta.abs().mean(-1) / (ttext.abs().mean(-1) + 1e-8)
        ...
        if (vca_layer.last_sigma is not None
            ...
            sig_t = vca_layer.last_sigma.float()
            T_t   = vca_layer.last_transmittance.float()
            ws = (T_t.unsqueeze(2) * sig_t).sum(dim=(2, 3))
        ...
        for fi in range(BF):
            ...
            if vca_layer.last_sigma is not None:
                sig_np = vca_layer.last_sigma[fi].cpu().numpy()
        ...
        if delta_gif:
            ...
            print(f"  [Phase 31][debug] vca_delta |Δ|/|txt| mean={mean_ratio:.3f} "
                  f"max={max_ratio:.3f}{warn}", flush=True)

    if vca_layer.last_sigma is not None:
        sig_np = vca_layer.last_sigma.float().cpu().numpy()
        ...
        if ray_gif:
            ...
            print(f"  [Phase 31][debug] ray3d internals → ray3d_epoch{epoch:03d}.gif", flush=True)
...
def _debug_utils(pipe, height):
    ...
    def _decode_frame(lat5d, fi):
        try:
            ...
            dec = pipe.vae.decode(inp).sample[0]
...
def debug_train_denoising(pipe, vca_layer, probe_latents, probe_enc_hs,
    ...
    """GIF: train_denoise.gif — GT / Noised / pred_x0."""
    import imageio.v2 as iio2
    from PIL import Image
    ...
    if gif_frames:
        ...
        print(f"  [Phase 31][debug] {p.parent.name}/train_denoise.gif", flush=True)
    vca_layer.train()
...
def debug_text_cond(pipe, vca_layer, probe_latents, probe_meta,
    ...
    if gif_frames:
        ...
        iio2.mimsave(str(p), gif_frames, duration=300)
        print(f"  [Phase 31][debug] {p.parent.name}/text_cond.gif", flush=True)
    vca_layer.train()
...
def debug_depth_effect(pipe, vca_layer, orig_procs, train_procs,
    ...
    for fi in range(min(T_frames, len(probe_frames_np))):
        ...
        for col_i, t_val in enumerate(t_values):
            ...
            x0_vca = _pred_x0(pipe, noisy, t, probe_enc_hs,
                              alphas_cumprod).clamp(-4, 4)
            sigma_now = vca_layer.last_sigma
    ...
    if gif_frames:
        ...
        iio2.mimsave(str(p), gif_frames, duration=300)
        print(f"  [Phase 31][debug] {p.parent.name}/depth_effect.gif", flush=True)
...
def debug_text_attn(pipe, probe_latents, probe_enc_hs, probe_meta,
    ...
    """
    Phase 31: LoRA removed. The processor at ATTN_CAPTURE_KEY is vanilla
    AttnProcessor2_0 which does not expose `last_weights`. We attempt to read
    from whatever processor is installed; if it has no `last_weights`, we
    skip attention visualization gracefully.
    """
    attn_proc_for_debug = dict(pipe.unet.attn_processors).get(ATTN_CAPTURE_KEY)
    if attn_proc_for_debug is None or not hasattr(attn_proc_for_debug, 'last_weights'):
        print(f"  [Phase 31][debug] text_attn skipped: "
              f"no `last_weights` on processor at {ATTN_CAPTURE_KEY} "
              f"(LoRA removed, vanilla AttnProcessor2_0 has no capture hook).",
              flush=True)
        return
    ...
    c0_tok_idx  = find_tok_idx(c0_name, full_ids)
    e0_tok_idx  = find_tok_idx(e0_kw,   full_ids)
    c1_tok_idx  = find_tok_idx(c1_name, full_ids)
    e1_tok_idx  = find_tok_idx(e1_kw,   full_ids)
    ...
    for fi in range(n_frames):
        ...
        for t_val in t_values:
            ...
            if hasattr(attn_proc_for_debug, 'last_weights') and attn_proc_for_debug.last_weights is not None:
                attn_weights_np = attn_proc_for_debug.last_weights
            W = attn_weights_np
            if W is None:
            ...
            W_mean = W_4d.mean(axis=1)

            fi_clamped = min(fi, BF - 1)
            w_fi = W_mean[fi_clamped]
            ...
            ov_c0 = _attn_to_overlay(attn_c0, gt_frame, (1.0, 0.50, 0.05))
            ov_e0 = _attn_to_overlay(attn_e0, gt_frame, (1.0, 0.10, 0.05))
            ov_c1 = _attn_to_overlay(attn_c1, gt_frame, (0.05, 0.60, 1.0))
            ov_e1 = _attn_to_overlay(attn_e1, gt_frame, (0.05, 0.15, 1.0))
    ...
    if gif_frames:
        ...
        iio2.mimsave(str(p), gif_frames, duration=300)
        print(f"  [Phase 31][debug] {p.parent.name}/text_attn.gif", flush=True)
    ...
    if entity_masks is not None:
        ...
        E0_COLOR_DARK  = '#e84545'
        E0_COLOR_LIGHT = '#ff9966'
        E1_COLOR_DARK  = '#3a7fff'
        E1_COLOR_LIGHT = '#66ccff'
        ...
        fig.savefig(str(chart_path), dpi=120, bbox_inches='tight',
                    facecolor='#0d0d1a')
        plt.close(fig)
        print(f"  [Phase 31][debug] {out_dir.name}/text_attn_chart.png", flush=True)
...
def debug_multiangle_depth(pipe, vca_layer, dataset, train_procs,
                           ...
                           out_dir: Path, height=256, width=256,
                           t_val=100):
    """GIF: multiangle_depth.gif — angle별 depth ordering 변화."""
    import imageio.v2 as iio2
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    ...
    for idx in range(len(dataset)):
        ...
        entity_masks = extra[0] if extra else None
    ...
    if not best_group:
        print("  [Phase 31][debug] multiangle_depth: no multi-angle group found", flush=True)
        return
    ...
    def _sigma_depth_scores(frames_np, entity_masks=None, meta=None):
        ...
        Z = sig.shape[3]
        z_weights = np.arange(Z, dtype=np.float32)
        scores = []
        for fi in range(sig.shape[0]):
            if entity_masks is not None and fi < entity_masks.shape[0]:
                m0 = entity_masks[fi, 0].astype(bool)
                m1 = entity_masks[fi, 1].astype(bool)
    ...
    def _save_depth_linechart(angle_data, out_dir):
        ...
        for ax, ad in zip(axes, angle_data):
            ...
            ax.invert_yaxis()
            ax.set_ylim(y_hi + 0.05, y_lo - 0.05)
            ax.grid(True, color='#222244', linewidth=0.5, alpha=0.6)
        ...
        fig.savefig(str(chart_path), dpi=120, bbox_inches='tight',
                    facecolor='#0d0d1a')
        plt.close(fig)
        print(f"  [Phase 31][debug] {out_dir.name}/multiangle_depth_chart.png", flush=True)
    ...
    if gif_frames:
        ...
        iio2.mimsave(str(p), gif_frames, duration=300)
        print(f"  [Phase 31][debug] {p.parent.name}/multiangle_depth.gif", flush=True)
...
def training_step_p21(pipe, vca_layer, latents, encoder_hidden_states,
    ...
    """
    Phase 31: LoRA / l_attn 제거. 순수 VCA training.
      l_total = lambda_diff * l_diff + lambda_depth * l_depth + lambda_ortho * l_ortho
    """
    noise = torch.randn_like(latents)
    t = torch.randint(0, t_max, (1,), device=device).long()
...
def train(args):
    ...
    print(f"[Phase 31] Backbone-dominance fix: gamma_init={VCA_ALPHA}, "
          f"lambda_diff={args.lambda_diff}, LoRA removed", flush=True)
    print(f"[Phase 31][init] device={device}  lambda_depth={args.lambda_depth}  "
          f"lambda_diff={args.lambda_diff}  alpha={VCA_ALPHA}", flush=True)
    ...
    print(f"[Phase 31][probe] '{e0_p}' vs '{e1_p}'  (color-qualified)", flush=True)

    # Additive VCA 주입
    vca_layer, orig_procs = inject_vca_p21(pipe, probe_entity_ctx)

    # Phase 31: LoRA 제거 — train_procs는 VCA processor dict 그대로
    ...
    print(f"[Phase 31][probe_prompt] '{full_probe_color}'", flush=True)

    trainable_vca  = [p for p in vca_layer.parameters() if p.requires_grad]

    # Learnable gamma in AdditiveVCAProcessor
    vca_proc = pipe.unet.attn_processors.get(INJECT_KEY)
    ...
    optimizer = torch.optim.AdamW(
        [{'params': trainable_vca,   'lr': args.lr,       'eps': 1e-8},
         {'params': trainable_gamma, 'lr': args.lr * 0.1, 'eps': 1e-8}],
        weight_decay=1e-4,
    )
    print(f"[Phase 31][opt] VCA: {sum(p.numel() for p in trainable_vca):,}  "
          f"gamma: {len(trainable_gamma)} scalar", flush=True)
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0 or epoch == args.epochs - 1:
            ...
            print(f"  [Phase 31][dra] depth_rank_accuracy={dra:.3f} ({n_correct}/{n_total})",
                  flush=True)
        else:
            dra = best_dra
        ...
        if (epoch + 1) % 10 == 0:
            ...
            if ld_on is not None:
                print(f"  [Phase 31][ablation] l_diff: VCA_on={ld_on:.4f}  VCA_off={ld_off:.4f}  "
                      f"ratio={ld_ratio:.3f}x  "
                      f"({'VCA_HELPS_DENOISING' if ld_ratio < 0.9 else 'BACKBONE_DOMINANT' if ld_ratio < 1.05 else 'VCA_AFFECTS_GENERATION' if ld_ratio < 1.3 else 'VCA_HURTS_BACKBONE'})",
                      flush=True)
        ...
        print(
            f"[Phase 31] epoch={epoch:3d} step={epoch_steps} "
        ...
        if probe_sep > best_sep:
            ...
            print(f"[Phase 31][ckpt] best.pt (probe_sep={best_sep:.4f} dra={best_dra:.3f})",
                  flush=True)
        ...
        if (epoch + 1) % args.debug_every == 0 or epoch == 0 or epoch == args.epochs - 1:
            ...
            # Phase 31: quantitative generation diff (baseline vs VCA)
    ...
    print(f"\n[Phase 31] FINAL probe_sep={best_sep:.6f}", flush=True)
    print(f"[Phase 31] FINAL depth_rank_accuracy={final_dra:.4f} ({fc}/{ft})", flush=True)
    ...
    print(f"[Phase 31][done]", flush=True)
...
def parse_args():
    ...
    p.add_argument("--save-dir",     default="checkpoints/phase31",    dest="save_dir")
    p.add_argument("--debug-dir",    default="debug/train_phase31",    dest="debug_dir")
