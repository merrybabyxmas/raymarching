...
def _set_requires_grad_stage_a(manager, vca_layer, procs):
    """Stage A: occ_heads + OBH + weight_head + slot_blend_raw 학습."""
...
def train_phase45(args):
    ...
    for epoch in range(total_epochs):
        ...
        if is_stage_a:
            loss_keys = ["total", "occ", "blend_target", "blend_rank",
                         "vis", "wrong", "w_res"]
        else:
            loss_keys = ["total", "occ", "vis", "wrong", "sigma", "depth", "ov",
        ...
        for batch_idx, data_idx in enumerate(step_indices):
            ...
            # ── Entity-only ref forwards (Stage B only) ──────────────────
            ...
            # ── L_occupancy (from OccupancyHead) ─────────────────────────
            # Sum over all blocks (each block has occ_heads)
            l_occ = torch.tensor(0.0, device=device)
            n_occ_blocks = 0
            for blk_idx, proc in enumerate(manager.procs):
                o0_fl = getattr(proc, 'last_o0_for_loss', None)
                o1_fl = getattr(proc, 'last_o1_for_loss', None)
                if o0_fl is None or not isinstance(o0_fl, torch.Tensor):
                    continue

                # Match spatial size to masks
                S_blk  = o0_fl.shape[1]
                ...
                n_frames_blk = min(o0_fl.shape[0], T_frames)
                for fi in range(n_frames_blk):
                    l_occ = l_occ + l_occupancy(
                        o0_fl[fi:fi+1].float(),
                        o1_fl[fi:fi+1].float(),
                        masks_blk[fi:fi+1])
                n_occ_blocks += max(n_frames_blk, 1)
            if n_occ_blocks > 0:
                l_occ = l_occ / n_occ_blocks
            ...
            if (bm_for_loss is not None and
                ...
                for fi in range(min(bm_for_loss.shape[0], T_use)):
                    ...
                    m_f  = masks_t[fi:fi+1]
                    l_bt = l_bt + l_blend_target_balanced(bm_f, m_f)
            ...
            if is_stage_a:
                ...
                for k, v in [("total", loss), ("occ", l_occ),
                              ("blend_target", l_bt), ("blend_rank", l_br),
                              ("vis", l_vis), ("wrong", l_wrong), ("w_res", l_w_res)]:
                ...
                for k, v in [("total", loss), ("occ", l_occ), ("vis", l_vis),
        ...
        print(f"[Phase 45][Stage {stage_lbl}] epoch {epoch:03d}/{total_epochs-1}  "
              ...
              f"vis={avg['vis']:.4f}  wrong={avg['wrong']:.4f}  w_res={avg['w_res']:.4f}",
              flush=True)
