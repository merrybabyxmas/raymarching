...
Phase 44 실패 원인:
  1. alpha-based prior가 overlap에서 반전 (exclusive > overlap)
  2. blend_rank 0.6대 고착 — global mean MSE로 loss 속임
  3. w0*w1는 visibility ambiguity proxy, overlap proxy가 아님

Phase 45 변경:
  Stage A: OccupancyHead (BCE by GT mask) + OBH + weight_head
    lr_occ=0.001, lr_obh=0.005 (Phase44 대비 각 5×, 25× 증가)
    eval_every_stage_a=1 (매 epoch 모니터링)
    loss: l_occ + l_blend_target_balanced + l_blend_rank + l_vis + l_wrong + l_w_res

  Stage B: 전체 학습 + l_occ 추가
"""
import argparse
import copy
import json
...
# Stage A (blend + occupancy)
DEFAULT_LA_OCC           = 2.0   # occupancy BCE (핵심)
DEFAULT_LA_OCC_STRUCT    = 1.5   # overlap/exclusive 구조 제약 (핵심)
DEFAULT_LA_BLEND_TARGET  = 2.0   # balanced MSE
DEFAULT_LA_BLEND_RANK    = 1.5   # ranking (Phase44 대비 강화)
DEFAULT_LA_VIS           = 0.5
...
DEFAULT_LR_OBH          = 5e-3   # Phase44 2e-4 → 5e-3 (25×)
DEFAULT_LR_OCC          = 1e-3   # NEW
...
def _load_soft_masks_from_seq_dir(
    ...
    """
    Load entity masks as soft occupancy via bilinear downsampling.

    Returns:
      (T, 2, hw*hw) float32 in [0, 1] or None if files are missing.
    """
...
def _get_soft_masks_for_sample(
    dataset,
    sample_idx:  int,
    hard_masks:  np.ndarray,   # (T, 2, S)
) -> np.ndarray:
    """
    Prefer soft masks from raw PNG files; fallback to provided hard masks.
    """
...
def _shift_with_fill(arr: np.ndarray, dx: int, dy: int, fill_value=0) -> np.ndarray:
    """Translate image/mask with zero/constant fill (no wrap-around)."""
...
def _try_collision_augment(
    ...
    """
    Build collision-heavy synthetic sample from pseudo-solo frames + raw masks.

    Returns:
      (frames_aug, depth_orders_aug, masks16_aug) or None.
    """
    if len(sample) < 8:
        return None
    if not hasattr(dataset, "samples") or sample_idx >= len(dataset.samples):
    ...
    T = min(frames_np.shape[0], len(depth_orders), entity_masks.shape[0], solo_e0.shape[0], solo_e1.shape[0])
    if T <= 0:
        return None
    ...
    # Candidate shifts: centroid align first, then random jitter.
...
def _frame_overlap_ratios_from_masks(
    ...
    """
    Per-frame overlap ratio from hard masks.
    masks_t2s: (T, 2, S) in {0,1}
    returns:   (T,) in [0,1]
    """
...
def _make_overlap_frame_weights(
    ...
    """
    Collision-aware frame weights.
    overlap가 희소한 setup에서 충돌 프레임 gradient를 증폭한다.
    """
...
def _dilate_entity_masks(
    ...
    """
    Binary/soft entity masks dilation on latent grid.
    """
...
@torch.no_grad()
def evaluate_rollout_p45(
    pipe,
    manager:    MultiBlockSlotManagerP45,
    dataset,
    val_idx:    list,
    device:     str,
    ...
    if ov.size > 0 and np.any(col_mask):
        col_w = w[col_mask] if w is not None else None
...
# =============================================================================
# Teacher-forced validation (Phase 45)
# =============================================================================

@torch.no_grad()
def evaluate_val_set_phase45(
    pipe,
    manager:    MultiBlockSlotManagerP45,
    vca_layer:  VCALayer,
    dataset,
    ...
    for vi in val_idx:
        try:
            ...
            for fi in range(T_frames):
                ...
                # blend stats (last_blend_map is (B, S) in Phase45)
    ...
    if not iou_e0s:
        return {"tf_iou_e0": 0.0, "tf_iou_e1": 0.0, "tf_ord": 0.0,
                ...
                "blend_gap_bg": 0.0,
                "occ_any_overlap_mean": 0.0,
                "occ_any_exclusive_mean": 0.0,
                "occ_any_bg_mean": 0.0,
                "occ_any_sep": 0.0,
                "occ_prod_overlap_mean": 0.0,
                "occ_prod_exclusive_mean": 0.0,
                "occ_prod_bg_mean": 0.0,
                "occ_prod_sep": 0.0}
    ...
    vs = val_score_phase45(tf_iou_e0, tf_iou_e1, tf_ord, tf_wrong,
                           blend_sep=blend_sep)
    return {
        "tf_iou_e0":            tf_iou_e0,
        "tf_iou_e1":            tf_iou_e1,
...
def _set_requires_grad_stage_a(manager, vca_layer, procs, slot_bootstrap: bool = True):
    ...
    for proc in procs:
        # occupancy heads — 핵심 Stage A 학습 대상
        ...
        if slot_bootstrap:
            # Slot identity bootstrap: occ head가 분리 가능한 feature를 보게 만든다.
...
def train_phase45(args):
    ...
    print("[Phase 45] 파이프라인 로드 중...", flush=True)
    pipe = load_pipeline(device=device, dtype=torch.float16)

    print(f"[Phase 45] checkpoint 로드: {args.ckpt}", flush=True)
    ...
    print("[Phase 45] 데이터셋 로드 중...", flush=True)
    try:
    ...
    procs, orig_procs = inject_multi_block_entity_slot_p45(
    ...
    manager = MultiBlockSlotManagerP45(procs, inject_keys, primary_idx=1)

    print("[Phase 45] checkpoint 복원...", flush=True)
    restore_multiblock_state_p45(manager, ckpt, device=device)
    ...
    print("\n[Phase 45] Epoch 0 no-op validation...", flush=True)
    vca_layer.eval(); manager.eval()
    val_m0 = evaluate_val_set_phase45(
    ...
    for epoch in range(total_epochs):
        ...
        if epoch == 0:
            print(f"\n[Phase 45] Stage A 시작 ({args.stage_a_epochs} epochs): "
                  f"OccupancyHead + blend head 검증", flush=True)
            ...
            print(f"\n[Phase 45] Stage B 시작 ({args.stage_b_epochs} epochs): "
        ...
        if is_stage_a:
            loss_keys = ["total", "occ", "occ_struct", "blend_target", "blend_rank",
                         "vis", "vis_iou", "wrong", "excl", "slot_ref", "slot_cont", "w_res"]
        else:
            loss_keys = ["total", "occ", "occ_struct", "vis", "vis_iou", "wrong", "sigma", "depth", "ov",
        ...
        for batch_idx, data_idx in enumerate(step_indices):
            ...
            # ── L_occupancy (primary block only) ─────────────────────────
            ...
            for blk_idx, _proc in enumerate(manager.procs):
                ...
                for fi in range(n_blk_frames):
                    ...
                    if F0_refs and F1_refs:
                        # slot-ref/contrast uses hard visibility semantics
        ...
        print(f"[Phase 45][Stage {stage_lbl}] epoch {epoch:03d}/{total_epochs-1}  "
              f"loss={avg['total']:.4f}  occ={avg['occ']:.4f}  "
              f"occ_struct={avg['occ_struct']:.4f}  "
        ...
        # ── Validation ────────────────────────────────────────────────────
        # Stage A: eval every epoch (빠른 모니터링)
        # Stage B: eval every eval_every epochs
        should_eval = (
        ...
        if should_eval:
            ...
            val_m = evaluate_val_set_phase45(
                pipe, manager, vca_layer, dataset, val_idx, device,
                t_fixed=args.t_max // 2,
                overlap_boost=args.val_overlap_boost,
                overlap_clip=args.val_overlap_clip,
            ...
            rollout_m = evaluate_rollout_p45(
            ...
            print(f"  [collision] frac={val_m['collision_frac']:.3f}  "
                  ...
                  f"roll_e1={rollout_m['rollout_collision_iou_e1']:.4f}", flush=True)
    ...
    print(f"\n[Phase 45] 완료. best epoch={best_epoch} val_score={best_val_score:.4f}",
          flush=True)
    print(f"  성공 기준: blend_sep > -0.10, blend_ov > 0.40, blend_ex < blend_ov", flush=True)
...
def main():
    p = argparse.ArgumentParser(description="Phase 45: Occupancy-Guided Blend Prior")

    p.add_argument("--ckpt",         type=str, default="checkpoints/phase44/best.pt")
    p.add_argument("--data-root",    type=str, default="toy/data_objaverse")
    p.add_argument("--save-dir",     type=str, default="checkpoints/phase45")
    p.add_argument("--debug-dir",    type=str, default="outputs/phase45_debug")
