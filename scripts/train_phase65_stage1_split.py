...
@torch.no_grad()
def evaluate_epoch(trainer: Stage1Trainer, loader: DataLoader, device: str) -> dict:
...
def main() -> int:
    ...
    out_dir = Path(cfg.get('output_dir', 'outputs/phase65/stage1_generalization'))
    out_dir.mkdir(parents=True, exist_ok=True)

    best_score = -1e9
    history = []
    for epoch in range(cfg['train'].get('stage1_epochs', 80)):
        epoch_metrics = []
        ...
        val_mean = evaluate_epoch(trainer, val_loader, device=device)
        ...
        print(f"[epoch {epoch + 1}] val_score={score:.4f} val_visible_iou_min={val_mean.get('visible_iou_min', 0.0):.4f}")
