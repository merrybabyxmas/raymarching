...
def visualize_sample(
    ...
    """Generate GIF for a single sample."""
    sample = dataset[sample_idx]
    frames = sample.frames  # (T, 3, H, W)
    visible = sample.visible_masks  # (T, 2, Hm, Wm)
    T = frames.shape[0]
    ...
    for t in range(T):
        frame_t = frames[t].unsqueeze(0).to(device)  # (1, 3, H, W)
