"""Generate GIF visualizations for holdout split samples."""
...
def create_comparison_frame(
    ...
    """Create side-by-side comparison: GT | Pred."""
...
def visualize_sample(
    ...
    """Generate frames for a single sample."""
...
def main() -> int:
    ...
    # Load split and get sample IDs
    splits = json.loads(Path(args.split_json).read_text())
    sample_ids = splits[args.split_name]
    id_to_idx = {p.name: i for i, p in enumerate(dataset.samples)}
    indices = [id_to_idx[x] for x in sample_ids if x in id_to_idx]
    ...
    selected_indices = np.linspace(0, len(indices) - 1, num_samples, dtype=int)

    for i, sel_idx in enumerate(selected_indices):
        idx = indices[sel_idx]
        sample_id = dataset.samples[idx].name
        output_path = output_dir / f"{sample_id}.gif"
        frames = visualize_sample(model, dataset, idx, device)
