from __future__ import annotations

import argparse
import json
from pathlib import Path

import torch
from torch.utils.data import DataLoader
import yaml

from phase65_min3d.adapters.decoder_adapter import DecoderSceneAdapter
from phase65_min3d.backbones.reconstruction_decoder import ReconstructionDecoder
from phase65_min3d.data import Phase65Dataset, phase65_collate_fn
from phase65_min3d.scene_module import SceneModule
from phase65_min3d.trainer_stage2 import Stage2Batch, Stage2Trainer


def _mean_metrics(epoch_metrics: list[dict]) -> dict:
    return {k: float(sum(m[k] for m in epoch_metrics) / max(len(epoch_metrics), 1)) for k in epoch_metrics[0].keys()}


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--stage1_ckpt", type=str, required=False, default=None)
    args = parser.parse_args()

    cfg = yaml.safe_load(Path(args.config).read_text())
    device = cfg.get("device", "cuda" if torch.cuda.is_available() else "cpu")

    dataset = Phase65Dataset(
        root=cfg["data"]["root"],
        image_size=cfg["data"].get("image_size", 256),
        mask_size=cfg["data"].get("mask_size", 64),
        num_frames=cfg["data"].get("num_frames", 16),
    )
    loader = DataLoader(dataset, batch_size=cfg["train"].get("batch_size", 2), shuffle=True, collate_fn=phase65_collate_fn)

    model = SceneModule(
        slot_dim=cfg["model"].get("slot_dim", 256),
        feat_dim=cfg["model"].get("feat_dim", 64),
        hidden_dim=cfg["model"].get("hidden_dim", 128),
        Hs=cfg["model"].get("Hs", 64),
        Ws=cfg["model"].get("Ws", 64),
        Hf=cfg["model"].get("Hf", 32),
        Wf=cfg["model"].get("Wf", 32),
    )
    if args.stage1_ckpt is not None:
        ckpt = torch.load(args.stage1_ckpt, map_location="cpu")
        model.load_state_dict(ckpt["model"], strict=False)

    adapter = DecoderSceneAdapter(
        feat_dim=cfg["model"].get("feat_dim", 64),
        out_dim=cfg["adapter"].get("decoder_out_dim", 128),
    )
    backbone = ReconstructionDecoder(in_dim=cfg["adapter"].get("decoder_out_dim", 128))
    trainer = Stage2Trainer(
        scene_module=model,
        adapter=adapter,
        backbone=backbone,
        device=device,
        lr_scene=cfg["train"].get("lr_scene_stage2", 5e-5),
        lr_adapter=cfg["train"].get("lr_adapter", 2e-4),
        lr_backbone=cfg["train"].get("lr_backbone", 1e-4),
        lambda_backbone=cfg["loss"].get("lambda_backbone", 1.0),
        lambda_vis=cfg["loss"].get("lambda_vis_stage2", 0.5),
        lambda_amo=cfg["loss"].get("lambda_amo_stage2", 0.5),
        lambda_temp=cfg["loss"].get("lambda_temp_stage2", 0.1),
        lambda_depth=cfg["loss"].get("lambda_depth_stage2", 0.05),
    )

    out_dir = Path(cfg.get("output_dir", "outputs/phase65/stage2"))
    out_dir.mkdir(parents=True, exist_ok=True)

    best_score = -1e9
    history = []
    for epoch in range(cfg["train"].get("stage2_epochs", 50)):
        prev_state = None
        epoch_metrics = []
        for batch in loader:
            frames = batch["frames"].to(device)
            visible = batch["visible_masks"].to(device)
            amodal = batch["amodal_masks"].to(device)
            _B, T = frames.shape[:2]
            for t in range(T):
                step_batch = Stage2Batch(
                    entity_names=batch["entity_names"][0],
                    text_prompt=batch["text_prompts"][0],
                    prev_frame=None if t == 0 else frames[:, t - 1],
                    gt_visible=visible[:, t],
                    gt_amodal=amodal[:, t],
                    gt_rgb=frames[:, t],
                    gt_front_idx=None,
                    t_index=t,
                )
                metrics, prev_state, _pred_rgb = trainer.step(step_batch, prev_state=prev_state)
                epoch_metrics.append(metrics)

        mean_metrics = _mean_metrics(epoch_metrics)
        mean_metrics["epoch"] = epoch + 1
        history.append(mean_metrics)
        score = trainer.evaluator.score_for_checkpoint(mean_metrics)

        latest_payload = {
            "scene_module": model.state_dict(),
            "adapter": adapter.state_dict(),
            "backbone": backbone.state_dict(),
            "config": cfg,
            "metrics": mean_metrics,
            "history": history,
            "best_score_so_far": best_score,
        }
        torch.save(latest_payload, out_dir / "latest_stage2.pt")

        if score > best_score:
            best_score = score
            best_payload = {
                "scene_module": model.state_dict(),
                "adapter": adapter.state_dict(),
                "backbone": backbone.state_dict(),
                "config": cfg,
                "metrics": mean_metrics,
                "history": history,
                "score": score,
            }
            torch.save(best_payload, out_dir / "best_stage2.pt")

        (out_dir / "history.json").write_text(json.dumps(history, indent=2))
        (out_dir / "last_metrics.json").write_text(json.dumps(mean_metrics, indent=2))
        print(f"[epoch {epoch+1}] score={score:.4f} metrics={mean_metrics}")

    return 0


if __name__ == "__main__":
    raise SystemExit(main())
