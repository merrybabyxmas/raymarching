...
DEFAULT_LAMBDA_ATTN   = 3.0    # Phase 26: 10x vs P25 (30.0은 depth 파괴 확인, 균형점)
DEFAULT_LR            = 5e-5
DEFAULT_EPOCHS        = 60
DEFAULT_T_MAX         = 200
...
def train(args):
    ...
    print(f"[Phase 26] lambda_attn={args.lambda_attn} (10x vs P25), clip_attn=0.5, lr_attn=full",
          flush=True)
    ...
    optimizer = torch.optim.AdamW(
        [{'params': trainable_vca,  'lr': args.lr,  'eps': 1e-8},
         {'params': trainable_attn, 'lr': args.lr,  'eps': 1e-3}],
        # Phase 26: attn lr = full lr (Phase 25에서 0.1× 사용시 gradient too weak)
        # eps=1e-3: fp16 weight + AdamW first step overflow 방지
        weight_decay=1e-4,
    )
    ...
    for epoch in range(args.epochs):
        ...
        for batch in loader:
            ...
            # Phase 26: attn clip 0.01→0.1 (lambda=30일 때 0.5는 depth 파괴, 0.1로 제한)
            torch.nn.utils.clip_grad_norm_(trainable_vca,  1.0)
            torch.nn.utils.clip_grad_norm_(trainable_attn, 0.1)
            # NaN grad 방지
...
def parse_args():
    ...
    p.add_argument("--save-dir",     default="checkpoints/phase26",    dest="save_dir")
    p.add_argument("--debug-dir",    default="debug/train_phase26",    dest="debug_dir")
