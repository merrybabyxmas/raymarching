...
Phase 22 문제:
  - text attention이 여전히 cat/dog를 구분 못함
    → "cat", "dog" 텍스트만으로는 색깔이 강한 비주얼 단서인 두 entity 분리 어려움
  - l_depth_ranking: ranking loss는 간접적 → z-bin 할당을 명시적으로 학습 안 함

Phase 23 수정:
  1. Color-qualified prompts:
     "a cat" → "a red cat",  "a dog" → "a blue dog"
     (meta.json color0/color1 RGB → 색이름 변환)
     Full prompt도 "a red cat and a blue dog" 로 변경
     → CLIP 임베딩이 색+객체 결합으로 더 discriminative
     → text_attn에서 color 토큰("red","blue") + entity 토큰 4개 모두 시각화

  2. Direct z-order loss (l_depth_ranking 대체):
     σ(front entity, z=0) 최대화 + σ(back entity, z=last) 최대화
     σ(front entity, z=last) 최소화 + σ(back entity, z=0) 최소화
     → z-bin 할당을 GT depth order로 직접 지도

핵심 측정 지표: depth_rank_accuracy (>85% 목표)
"""
import argparse
import copy
import json
...
def training_step_p21(pipe, vca_layer, latents, encoder_hidden_states,
    ...
    entity_masks: (T, 2, S) bool numpy — 16×16 flatten entity spatial masks.
      l_zorder_direct에 전달 → entity 위치 spatial token만 depth loss 반영.
    """
    noise = torch.randn_like(latents)
...
def parse_args():
    ...
    p.add_argument("--save-dir",     default="checkpoints/phase23",    dest="save_dir")
    p.add_argument("--debug-dir",    default="debug/train_phase23",    dest="debug_dir")
