...
class AdditiveVCAProcessor:
    ...
    def __init__(self, vca_layer: VCALayer, entity_ctx: torch.Tensor,
        ...
        self.alpha = alpha
        self.last_delta_ratio: float = 0.0
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None,
        ...
        # diagnostic: actual |vca_delta| / |text_out| ratio (should stay ≈ alpha by normalization)
        with torch.no_grad():
...
def measure_ldiff_ablation(pipe, vca_layer, dataset, device,
    ...
    for idx in indices:
        ...
        # VCA off (alpha=0 → delta=0, pure backbone)
        proc.alpha = 0.0
        vca_layer.reset_sigma_acc()
        with torch.autocast(device_type="cuda", dtype=torch.float16):
...
def train(args):
    ...
    print(f"[Phase 29] LoRA-based text attn (rank=4, fp32), lambda_attn={args.lambda_attn}",
          flush=True)
    ...
    print(f"[p29] LoRA attn2 (q_dim={q_dim}, ctx={context_dim}, rank=4): "
...
def parse_args():
    ...
    p.add_argument("--save-dir",     default="checkpoints/phase29",    dest="save_dir")
    p.add_argument("--debug-dir",    default="debug/train_phase29",    dest="debug_dir")
